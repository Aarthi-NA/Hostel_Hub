...
const getHostelAttendance = async (req, res) => {
    ...
    try {
        ...
        const attendance = await Attendance.find({ student: { $in: students }, date: { $gte: date.setHours(0, 0, 0, 0), $lt: date.setHours(23, 59, 59, 999) } }).populate('student', ['_id','name', 'room_no', 'roll_no']);
        success = true;
        res.status(200).json({ success, attendance });
    }
    catch (err) {
