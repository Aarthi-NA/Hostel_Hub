...
router.post('/register-student', [
    ...
    check('roll_no', 'ROLL NO of at least 6 digit is required').isLength(6),
    ...
    check('aadhar', 'Enter valid Aadhar').isLength(13),
    check('hostel', 'Hostel is required').not().isEmpty(),
    check('password', 'Please enter a password with 8 or more characters').isLength({ min: 8 }),
], registerStudent);

// @route  POST api/student/get-student
// @desc   Get student by ROLL NO
// @access Public
...
router.post('/update-student', [
    check('roll_no', 'ROLL NO is required').not().isEmpty(),
    ...
    check('address', 'Address is required').not().isEmpty(),
    check('dob', 'Date of birth is required').not().isEmpty(),
    check('aadhar', 'Aadhar is required').not().isEmpty(),
